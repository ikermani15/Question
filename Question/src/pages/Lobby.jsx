...
function Lobby() {
  ...
  const { group, participant, leaveGroup, enterGroup } = useGroup()
  const { lang }                           = useLang()
  const t                                  = lang === "en"
  ...
  async function handleKick(p) {
    ...
    try {
      await kickParticipant(p.id, group.id, participant.username, groupInfo.creator_username)
      loadLeaderboard()
    } catch (err) {
      alert(err.message)
    }
  ...
  async function handleDelete() {
    try {
      await deleteGroup(group.id, participant.username, groupInfo.creator_username)
      leaveGroup()
      navigate("/grupos")
    } catch (err) {
      alert(err.message)
    ...
    <div className="min-h-screen bg-gray-900 px-4 py-8">
      <div className="w-full max-w-xl mx-auto">
        ...
        <div className="mb-8">
          ...
          {loading ? (
            <p className="text-gray-500 text-center py-4">{t ? "Loading..." : "Cargando..."}</p>
          ) : leaderboard.length === 0 ? (
            <p className="text-gray-500 text-center py-4">{t ? "No players yet" : "Aún no hay jugadores"}</p>
          ) : (
              ...
              {leaderboard.map((player, index) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {/* Botón expulsar — solo visible para el creador y no sobre sí mismo */}
              ...
              {!confirmDelete ? (
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 py-3 bg-red-600 hover:bg-red-500 text-white text-sm font-semibold rounded-xl transition-colors"
                  >
                    {t ? "Confirm delete" : "Confirmar eliminación"}
                  </button>
                  <button
                    onClick={() => setConfirmDelete(false)}
                    className="flex-1 py-3 bg-gray-800 text-gray-300 text-sm rounded-xl transition-colors"
                  >
                    {t ? "Cancel" : "Cancelar"}
                  </button>
                </div>
